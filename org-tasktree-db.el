;;; org-tasktree-db.el --- SQLite helpers for org-tasktree -*- lexical-binding: t; -*-
...
(defconst org-tasktree--sqlite-foreign-keys-pragma
  "PRAGMA foreign_keys = ON;"
  "PRAGMA to enable SQLite foreign key constraints for the connection.")
...
(defun org-tasktree-db--ensure-parent-dir (path)
  ...
  (let ((dir (file-name-directory (org-tasktree-db--normalize-path path))))
...
The caller must close the returned connection using `org-tasktree-db--close`."
  (let ((db (sqlite-open (org-tasktree-db--database-path))))
    (sqlite-execute db org-tasktree--sqlite-foreign-keys-pragma)
    db))
...
(defmacro org-tasktree-db--with-transaction (db &rest body)
  "Execute BODY in a transaction on DB."
  (declare (indent 1))
  `(progn
...
value TEXT NOT NULL\
             ...
             "CREATE UNIQUE INDEX IF NOT EXISTS idx_nodes_uid ON nodes(uid);"
             "CREATE UNIQUE INDEX IF NOT EXISTS idx_nodes_project_title ON nodes(title) WHERE node_type = 'project';"
             "CREATE UNIQUE INDEX IF NOT EXISTS idx_nodes_phase_title ON nodes(project_id, title) WHERE node_type = 'phase';"
             "CREATE UNIQUE INDEX IF NOT EXISTS idx_nodes_task_title ON nodes(parent_id, title) WHERE node_type = 'task';"
             "CREATE INDEX IF NOT EXISTS idx_nodes_status_scheduled ON nodes(status, scheduled);"
             "CREATE INDEX IF NOT EXISTS idx_nodes_status_deadline ON nodes(status, deadline);"
...
(defun org-tasktree-db--ensure-inbox (db)
  "Ensure the system Inbox project exists and is valid." 
  (let ((now (format-time-string "%FT%T%:z" (current-time))))
    (sqlite-execute
     db
 ...
 NULL, NULL, ?, ?);"
     (vector org-tasktree-db--inbox-id org-tasktree-db--inbox-uid now now))
    (let ((rows (sqlite-select db "SELECT id, uid, node_type, title, status FROM nodes WHERE id = ?;" (vector org-tasktree-db--inbox-id))))
      (unless rows
...
This function is idempotent."
  (org-tasktree-db--ensure-parent-dir (org-tasktree-db--database-path))
